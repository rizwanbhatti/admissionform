...
export default function MainPage() {
    ...
    const handleImageUpload = (imageUrl) => {
        console.log("image form Uploader-->", imageUrl)
        setFormData((prevData) => ({
            ...prevData,
            imageUrl, // Update the formData with the new image URL
        }));
        setResetImage(false);
    };
  ...
  formData.email.includes('.') && // Any domain extension like .com, .net, etc.
  !formData.email.includes(' ') &&
  formData.email.indexOf('@') > 0 && // '@' should not be the first character
  formData.email.lastIndexOf('.') > formData.email.indexOf('@') + 1 // '.' should come after '@'
        ) {
    ...
    return (
        <div className="mr-0 mb-0 ml-0 pb-8 relative bg-[#c1ebee]">  {/* Parent div with background color */}
            ...
            <div style={{ boxShadow: '1px 5px 5px 8px rgba(0.2, 0.2, 0.2, 0.2)' }} className=" mt-8 mx-auto h-[280px] lg:h-[350px] md:h-[350px] sm:h-[350px] w-[95%]  lg:w-[60%] md:w-[60%] mx:w-[60%] rounded-xl mb-[30px] transform transition-all duration-500 hover:scale-105 hover:shadow-xl">
                {admin?.infoImage?
                <Image className="h-[280px] lg:h-[350px] md:h-[350px] sm:h-[350px] mx-auto w-full rounded-xl" src={admin?.infoImage} alt="course info" width={600} height={400}
                // /images/Rizwan.png
                />
                :null
}
    ...
    `}
                    ...
                    <ImageUploader ratio={[3,4]} size="200px" formImage={formData.imageUrl} onImageUpload={handleImageUpload} reset={resetImage} />
            ...
            <div>
                <IdCardModal isOpen={showModal} onClose={closeModal} user={currentUser} contentImage={contentImage} />
            </div>
